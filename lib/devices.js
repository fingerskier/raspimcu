...
async function isBoardStorage(volumePath) {
  ...
  // Try to read INFO_UF2.TXT directly - single I/O operation
  let infoContents = '';
  try {
    infoContents = await fs.readFile(infoFile, 'utf8');
  } catch (error) {
    // File doesn't exist or isn't readable, check for INDEX.HTM instead
...
async function findMountedBoards(searchRoots = getDefaultSearchRoots()) {
  const scanResults = await Promise.all(searchRoots.map(scanRoot));
  const results = scanResults.flat();
...
async function listDevices(options = {}) {
  ...
  const storageRoots = searchRoots ?? getDefaultSearchRoots();
  if (Array.isArray(storageRoots) && storageRoots.length === 0) {
    result.errors.push({
      source: 'storage',
      error: {
        message: 'No search roots were provided for mounted board detection.'
      }
    });
  } else {
    try {
      const mountedBoards = await findMountedBoards(storageRoots);
...
async function getSingleDevice(options = {}) {
  ...
  if (devices.length === 0) {
    return { device: null, count: 0, error: 'No devices found' };
  }
